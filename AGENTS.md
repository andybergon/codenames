...
- The hidden `?mode=calibrate` page owns versioned blinded human rounds. Keep public definitions under `public/data/calibration/`, separate model and role answer keys under `scripts/generated/calibration-answer-keys/`, answers in the versioned local storage record, and JSON import/export compatible so answers can be corrected or later rounds added. Treat the first single-rater round as a gross-failure disqualifier, not a model ranker.
