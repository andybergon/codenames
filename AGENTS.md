...
- Completed Play links use the versioned `g` export owned by `src/play/game-share.js`. Preserve old decoders when rules or assets change, use explicit board data so recent-word history is irrelevant, and rebuild derived turn and outcome events through validated replay.
