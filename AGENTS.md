...
- Hosted embedding experiments must use the cached, cost-capped `npm run experiment:api-index` path and the human-data `npm run evaluate:api-embeddings` gate. Keep generated vectors under `.cache`; check in only compact reports and conclusions.
- The production Play defaults are BGE-small, 10,000 candidates, hybrid scoring, a five-point multi-clue tolerance, and passing at the declared clue number. Keep each parameter overridable in Play setup and persisted in the saved game.
