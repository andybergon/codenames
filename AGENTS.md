...
- Reconstruct post-game clue turns from ordered event history. Completion-gate replay and operative score overlays so live operative views never expose intended targets, hidden roles, or analysis.
...
- Italian E5 can rank orthographic false friends above semantic relationships after centering. Keep `MONOLOGO → MONGOLFIERA` and `PARTONO → PANTERA + BURATTINO` as regression fixtures when changing Italian assets, centering, or clue policy.
...
- The hidden `?mode=calibrate` page owns versioned blinded human rounds. Keep public definitions under `public/data/calibration/`, separate model and role answer keys under `scripts/generated/calibration-answer-keys/`, and JSON import/export compatible so answers can be corrected or later rounds added. Guesses, ratings, and notes persist to the versioned local storage record as they change and sync through `api/calibration.js` when `DATABASE_URL` and `CALIBRATION_SYNC_SECRET` are configured. Keep the production sync key server-side, use the HTTP-only pairing cookie, and let the loopback Vite middleware trust local requests without a UI key. Retain local-only behavior when the database is unavailable. Empty passes require an explicit action. Treat the first single-rater round as a gross-failure disqualifier, not a model ranker.
