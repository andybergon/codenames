...
- Completed Play links use the versioned `g` export owned by `src/play/game-share.js`. Version format, Play rules, and settings schemas separately; do not use a raw build ID as the compatibility key. Preserve old decoders, explicit board data, action context, and final outcome. Unsupported historical rules or settings must retain a history-only review and the original export code even when current analysis cannot run.
