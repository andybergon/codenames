...
| 🔀 Pairwise reranker | ❌ Fails | Regresses | +24 MB | `$0` |
| 🧪 Bridge + rerank | ✅ Passes | Marginal | +104 MB RAM | `$0` |
| ☁️ Hosted listwise | 🚫 Ineligible | Not run | Network | `$0` |

The latent walk searched the top 64 local clue-vocabulary concepts and used the strongest positive two-edge cosine product. It still left `PIANO` ahead of `GLOVE`, `MATCH`, and `BELT`. The reranker ablation uses the Apache-2.0 `Xenova/ms-marco-MiniLM-L-6-v2` quantized ONNX model at pinned revision `a09144355adeed5f58c8ed011d209bf8ee5a1fec`. The raw pairwise model ranks `PIANO` first for JOUST and performs substantially worse than both direct BGE and the WordNet bridge. The bounded pipeline first applies the production BGE activation and WordNet expansion, then gives the top eight candidates a normalized cross-encoder adjustment capped at `0.04`. It preserves JOUST but still requires WordNet.

The local cross-encoder made a paid hosted comparison unnecessary. A hosted listwise model would remain comparison-only because automatic turns must work offline with bounded latency and no per-turn spend. No API was called and the measured paid cost was `$0`.
...
The direct pairwise reranker scores 107,624 unique clue-card pairs and fails every human gate. Cultural Codes target recall falls to `48.39%`, exact target sets to `42.48%`, and avoid-word rate rises to `11.93%`; Connector target recall falls to `44.58%`. The strongest `0.04` bounded bridge-rerank pipeline reaches `59.21%` Cultural Codes target recall, `53.19%` exact target sets, and `9.39%` avoid-word rate. Connector target recall stays at `53.76%`, exact target sets move from `25.24%` to `25.29%`, and pairwise target accuracy declines from `82.86%` to `82.80%`. These movements are too small and mixed to justify another runtime model.
...
The pinned reranker cache is `23,856,961` bytes. In a fresh process with the model already downloaded, first activation took `61ms`, the first 25-pair inference took `5.1ms`, and resident memory rose by about `105MB`. Warm direct scoring took `4.3ms` for 25 raw pairs; the eight-item bridge shortlist took `5.7ms`, in addition to the existing `115ms` median WordNet embedding work. The [20-board 30,000-candidate full-game comparison](../scripts/generated/bridge-reranker-full-game-comparison.json) produced zero decision or aggregate deltas against the WordNet bridge, including identical `1.642105` correct cards per turn, zero wrong-team, neutral, assassin, fallback, or stall events, and `9.5` turns per game.

An [unrestricted MiniLM operative experiment](../scripts/generated/concept-ranking-unrestricted-cross-model-comparison.json) increased correct cards per turn by `0.079` but also increased wrong-team hits by `0.57` per game, neutral hits by `0.49`, and assassin rate by `0.06`. It failed all three transfer safety gates. Production therefore enables the bridge only for BGE-small and preserves direct ranking for every other operative model. The final [100-board guarded cross-model comparison](../scripts/generated/concept-ranking-cross-model-comparison.json) confirms that requesting concept ranking with a MiniLM operative produces the exact direct baseline. The [20-board guarded reranker transfer check](../scripts/generated/bridge-reranker-cross-model-comparison.json) also produces zero paired deltas across correct cards, wrong-team hits, neutral hits, assassin rate, fallbacks, stalls, and turns because unsupported operative geometries fail closed to direct ranking.

The WordNet runtime bridge remains required. The direct reranker cannot replace it, and the bounded reranker cannot operate without it. The small mixed human changes, zero full-game benefit, added download and memory, and separate retrieval-oriented training objective prevent promotion. Production behavior therefore remains unchanged.
...
| 🔗 `npm run evaluate:concept-ranking` | [`concept-ranking-evaluation.json`](../scripts/generated/concept-ranking-evaluation.json) | JOUST, bridge/reranker ablations, human fit, latency, memory |
| 🎮 `npm run benchmark:compare` | [`bridge-reranker-full-game-comparison.json`](../scripts/generated/bridge-reranker-full-game-comparison.json) | Bounded reranker full-game effects |
| 🛡️ `npm run benchmark:compare` | [`bridge-reranker-cross-model-comparison.json`](../scripts/generated/bridge-reranker-cross-model-comparison.json) | Guarded transfer fallback |
