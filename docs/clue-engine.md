...
The hidden `?mode=calibrate` page loads versioned rounds from `public/data/calibration/manifest.json`. It automatically stores each guess, rating, and note under `codenames-human-calibration-v1`, while an empty pass requires the explicit Record pass action. When `DATABASE_URL` and `CALIBRATION_SYNC_SECRET` are configured, `/api/calibration` also upserts each answer into `codenames_calibration_answers` and restores the newest answer by timestamp. The pairing key is exchanged once for a strict HTTP-only cookie and is never stored in browser-readable storage. Browser persistence and JSON import/export remain available when database sync is unavailable.
