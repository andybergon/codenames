...
The morphology filter uses explicit English suffix transformations plus a generated Princeton WordNet relation index rather than generic substring matching. `npm run generate:concepts` extracts irregular inflections from WordNet exception lists and direct derivational or pertainym relations from lexical pointers. Lexical-pointer pairs must also share a substantial spelling base, which keeps semantic alternatives such as `eye` / `optical` and `tooth` / `dental` legal. This rejects regular pairs such as `life` / `lives`, `story` / `stories`, `run` / `running`, and `teach` / `teacher`, plus irregular or derivational pairs such as `mouse` / `mice`, `Rome` / `Roman`, and `spine` / `spinal`, without treating unrelated pairs such as `plane` / `planet` as equivalent. Ranked Train and Play suggestions, benchmark fallback clues, and manually entered Play clues share this legality rule. The generated index is deterministic and local-first, but it does not replace table-specific spymaster rulings.

Play rules version 3 introduces the generated English relation index. The canonical game state retains its original rules version so share decoding, saved-session restoration, undo, and forward replay apply the legality capabilities that existed when the game was created. Capability gates use fixed introduction versions, which preserves v1 and v2 clues when later rules versions are added.
