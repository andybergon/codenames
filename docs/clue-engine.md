...
The hidden `?mode=calibrate` page loads versioned rounds from `public/data/calibration/manifest.json`. It automatically stores each guess, rating, and note under `codenames-human-calibration-v1`, while an empty pass requires the explicit Record pass action. When `DATABASE_URL` and `CALIBRATION_SYNC_SECRET` are configured, `/api/calibration` also upserts each answer into `codenames_calibration_answers` and restores the newest answer by timestamp. Production exchanges the pairing key once for a strict HTTP-only cookie and never stores it in browser-readable storage. The loopback Vite middleware trusts local requests and uses the server-side `DATABASE_URL` directly, so local calibration does not require pairing. Browser persistence and JSON import/export remain available when database sync is unavailable.
