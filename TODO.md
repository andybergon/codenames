...
- Evaluate a more game-specific semantic embedding or reranker.
  - Current: Model picker spans 17.5 MB MiniLM-L3 through 110 MB MPNet-base, with MiniLM-L6, BGE-small, and MiniLM-L12 between them. Every model has compatible lazy-loaded indexes, measured human target recall, model size, and controlled median Node scoring time. BGE-small has the best played-turn target recall; MiniLM-L12 is stronger on intended pairs but makes more avoid-word errors.
...
- Review larger clue indexes before changing the 3,000-candidate default.
  - Current: Model picker exposes incremental 3k, 10k, and 30k int8 shards for every selectable model. Exact coverage of 9,932 usable human clues is 61.8%, 85.1%, and 93.5%; scoring speed uses a controlled repeated Node benchmark.
